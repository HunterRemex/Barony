...
void Creature::vampireChooseWeapon(const Entity* target, double dist)
{
	...
	if ( monsterSpecialTimer == 0 && (ticks % 10 == 0) && monsterAttack == 0 )
	{
		...
		specialRoll = local_rng.rand() % (20 + 50 * (targetCrtr->behavior == &actMonster));
		if ( myStats->HP <= myStats->MAXHP * 0.8 )
		{
...
void Creature::handleEffects(Stat* myStats)
{
	...
	if ( myStats->EXP >= 100 )
	{
		...
		if ( !(behavior == &actMonster && monsterAllySummonRank != 0) )
		{
			...
			if ( behavior == &actPlayer && myStats->playerRace == RACE_INSECTOID && myStats->appearance == 0 )
			{
				...
				{
					int32_t hungerPointPerMana = playerInsectoidHungerValueOfManaPoint(*myStats);
		...
		if ( multiplayer == SERVER )
		{
			if ( player > 0 && !players[player]->isLocalPlayer() )
			{
				...
				net_packet->data[5] = (int8_t)myStats->STR;
				net_packet->data[6] = (int8_t)myStats->DEX;
				net_packet->data[7] = (int8_t)myStats->CON;
				net_packet->data[8] = (int8_t)myStats->INT;
				net_packet->data[9] = (int8_t)myStats->PER;
				net_packet->data[10] = (int8_t)myStats->CHR;
				net_packet->data[11] = (int8_t)myStats->EXP;
				net_packet->data[12] = (int8_t)myStats->LVL;
				SDLNet_Write16((int16_t)myStats->HP, &net_packet->data[13]);
				SDLNet_Write16((int16_t)myStats->MAXHP, &net_packet->data[15]);
				SDLNet_Write16((int16_t)myStats->MP, &net_packet->data[17]);
				SDLNet_Write16((int16_t)myStats->MAXMP, &net_packet->data[19]);
				net_packet->address.host = net_clients[player - 1].host;
				net_packet->address.port = net_clients[player - 1].port;
				net_packet->len = 21;
				sendPacketSafe(net_sock, -1, net_packet, player - 1);
				...
				net_packet->data[4] = clientnum;
				net_packet->data[5] = (uint8_t)myStats->PLAYER_LVL_STAT_TIMER[STAT_STR];
				net_packet->data[6] = (uint8_t)myStats->PLAYER_LVL_STAT_TIMER[STAT_DEX];
				net_packet->data[7] = (uint8_t)myStats->PLAYER_LVL_STAT_TIMER[STAT_CON];
				net_packet->data[8] = (uint8_t)myStats->PLAYER_LVL_STAT_TIMER[STAT_INT];
				net_packet->data[9] = (uint8_t)myStats->PLAYER_LVL_STAT_TIMER[STAT_PER];
				net_packet->data[10] = (uint8_t)myStats->PLAYER_LVL_STAT_TIMER[STAT_CHR];
				net_packet->data[11] = (uint8_t)myStats->PLAYER_LVL_STAT_TIMER[STAT_STR + NUMSTATS];
				net_packet->data[12] = (uint8_t)myStats->PLAYER_LVL_STAT_TIMER[STAT_DEX + NUMSTATS];
				net_packet->data[13] = (uint8_t)myStats->PLAYER_LVL_STAT_TIMER[STAT_CON + NUMSTATS];
				net_packet->data[14] = (uint8_t)myStats->PLAYER_LVL_STAT_TIMER[STAT_INT + NUMSTATS];
				net_packet->data[15] = (uint8_t)myStats->PLAYER_LVL_STAT_TIMER[STAT_PER + NUMSTATS];
				net_packet->data[16] = (uint8_t)myStats->PLAYER_LVL_STAT_TIMER[STAT_CHR + NUMSTATS];
				net_packet->address.host = net_clients[player - 1].host;
				net_packet->address.port = net_clients[player - 1].port;
				net_packet->len = 17;
				sendPacketSafe(net_sock, -1, net_packet, player - 1);
	...
	// hunger
	int hungerTickRate = Creature::getHungerTickRate(myStats, behavior == &actPlayer, true);
	...
	else if ( ticks % hungerTickRate == 0 )
	{
		...
		{
			myStats->HUNGER--;
			int32_t noLongerFull = getEntityHungerInterval(player, this, myStats, HUNGER_INTERVAL_OVERSATIATED);
			int32_t youFeelHungry = getEntityHungerInterval(player, this, myStats, HUNGER_INTERVAL_HUNGRY);
			int32_t youFeelWeak = getEntityHungerInterval(player, this, myStats, HUNGER_INTERVAL_WEAK);
			int32_t youFeelFaint = getEntityHungerInterval(player, this, myStats, HUNGER_INTERVAL_STARVING);
	...
	{
		int manaRegenInterval = Creature::getManaRegenInterval(this, *myStats, behavior == &actPlayer);
		this->char_energize++;
		...
		else if ( myStats->HUNGER > 1200 )
		{
			...
			if ( myStats->MP / static_cast<double>(std::max(1, myStats->MAXMP)) <= 0.5 )
			{
	...
	else if ( this->behavior == &actPlayer && myStats->playerRace == RACE_INSECTOID && myStats->appearance == 0 )
	{
		if ( (svFlags & SV_FLAG_HUNGER) && !MFLAG_DISABLEHUNGER )
		{
			...
			if ( this->char_energize > 0 && this->char_energize % 5 == 0 ) // check every 5 ticks.
			{
				...
				int32_t Sint32expectedMana = static_cast<int32_t>(expectedManaValue);
				if ( myStats->HUNGER > 0 )
				{
	...
	if ( myStats->shield != NULL )
	{
		if ( myStats->shield->type == TOOL_TORCH || myStats->shield->type == TOOL_LANTERN )
		{
			...
			if ( (this->char_torchtime >= 7500 && myStats->shield->type == TOOL_TORCH) || (this->char_torchtime >= 10500) )
			{
				...
				{
					std::basic_string<char> itemName = myStats->shield->getName();
	...
	if ( myStats->EFFECTS[EFF_POISONED] )
	{
		...
		if ( this->char_poison > 150 )   // three seconds
		{
            ...
            Creature* killerCrtr = (Creature*)(*&killer);
			if ( myStats->HP <= 0 )
			{
				if ( Creature* creature = dynamic_cast<Creature*>(killer); creature )
				{
					creature->awardXP(this, true, true);
				}
				else
				{
			...
			if ( killerCrtr && killerCrtr->behavior == &actPlayer )
			{
	...
	if ( myStats->EFFECTS[EFF_BLEEDING] )
	{
		if ( ticks % 120 == 0 )
		{
			if ( myStats->HP > 5 + (std::max(0, getCON())) ) // CON increases when bleeding stops.
			{
				...
				Creature* killer = uidToCreature(static_cast<uint32_t>(myStats->bleedInflictedBy));
	...
	if ( myStats->EFFECTS[EFF_WEBBED] )
	{
		...
		if ( ticks % 40 == 0 )
		{
			...
			if ( entity != NULL )
			{
				...
				entity->sizey = 2;
				entity->yaw = (local_rng.rand() % 360) * PI / 180.0;
				double scale = 0.75 + 0.25 * (local_rng.rand() % 100) / 100.f;
				entity->scalex = scale;
				entity->scaley = scale;
				entity->flags[UPDATENEEDED] = true;
				entity->flags[PASSABLE] = true;
	...
	if ( this->flags[BURNING] )
	{
		...
		else
		{
			...
			if ( (this->char_fire % TICKS_TO_PROCESS_FIRE) == 0 )
			{
				...
				if ( buddhamode )
				{
					int32_t fireDamage = (-2 - local_rng.rand() % 3); // Deal between -2 to -5 damage
				...
				else
				{
					...
					Entity* killer = uidToEntity(static_cast<uint32_t>(myStats->burningInflictedBy));
                    Creature* killerCrtr = (Creature*)(*&killer);
					...
					if ( myStats->HP <= 0 )
					{
						...
						{
							if (achievementObserver.checkUidIsFromPlayer(static_cast<uint32_t>(myStats->burningInflictedBy)) >= 0 )
							{
...
void Creature::monsterHandleKnockbackVelocity(double monsterFacingTangent, double weightratio)
{
	...
	double maxVelX = cos(monsterFacingTangent) * .045 * (monsterGetDexterityForMovement() + 10) * weightratio;
	double maxVelY = sin(monsterFacingTangent) * .045 * (monsterGetDexterityForMovement() + 10) * weightratio;
